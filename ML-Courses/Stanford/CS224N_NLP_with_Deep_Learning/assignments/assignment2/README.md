...
(1) Please repeat parts (b) and (c), computing the partial derivatives of $J_{neg-sample}$ with respect to $v_c$, with repsect to $u_o$, and with respect to the $s^{th}$ negative sample $u_{w_s}$. Please write your answers in terms of the vectors $v_c$, $u_o$, and $u_{w_s}$, where $s \in [1, K]$. *Note: you should be able to use your solution to part (6) to help compute the necessary gradients here.*
